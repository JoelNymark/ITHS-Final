    ...
    def simulateEpisodes(self):
        for episode in range(self.numberEpisodes):
            ...
            while not done:
                ...
                action = self.selectAction(state, episode)
                next_state, reward, done, _, _ = self.env.step(action)
                rewards.append(reward)
                next_state_index = self.returnIndexState(next_state)
                Qmax = np.max(self.Qmatrix[next_state_index])
                if not done:
                    error = reward + self.gamma * Qmax - self.Qmatrix[state_index + (action,)]
                else:
                    error = reward - self.Qmatrix[state_index + (action,)]
                self.Qmatrix[state_index + (action,)] += self.alpha * error
                state = next_state
